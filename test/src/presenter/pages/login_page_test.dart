...
import 'package:tasky_clean_cubit/src/presenter/pages/login_page.dart';
...
class _DummyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: Center(child: Text(label, textDirection: TextDirection.ltr)));
  }
}
...
GoRouter _router() {
  return GoRouter(
    ...
    routes: [
      ...
      GoRoute(
          path: '/register', builder: (_, __) => const _DummyPage('Register')),
    ],
  );
}
...
void main() {
  ...
  testWidgets('calls login with valid credentials', (tester) async {
    ...
    await tester.enterText(find.byType(TextFormField).at(0), 'john@dev.io');
  ...
  testWidgets('shows SnackBar on AuthFailure', (tester) async {
    ...
    auth.emit(const AuthFailure('Login failed: boom'));
    await tester.pump();
    await tester.pump(const Duration(seconds: 1));

    expect(find.text('Login failed: boom'), findsOneWidget);
  });

  testWidgets(
      'navigates to /tasks and shows welcome SnackBar on AuthAuthenticated',
      (tester) async {
    final auth = getIt<AuthCubit>() as _AuthCubitFake;
    final router = _router();
    await tester.pumpWidget(_app(router, auth));
    ...
    auth.emit(
        const AuthAuthenticated(AppUser(id: '1', email: 'a@b.c', name: 'A')));
    await tester.pumpAndSettle();
  ...
  testWidgets('Create account link navigates to /register', (tester) async {
    final auth = getIt<AuthCubit>() as _AuthCubitFake;
    final router = _router();
    await tester.pumpWidget(_app(router, auth));
    await tester.pumpAndSettle();
